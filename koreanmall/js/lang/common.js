...
function board_popup(width, height) {

	openWindow('/app/board/login_pop', 'mall_login_pop', 'resizable=yes,scrollbars=yes', width, height, true);
}
...
function order_claim_cmd()
{
	ff = document.lf1;

	var ord_no = ff.ord_no.value;
	var claim_reason = ff.claim_reason.value;
	var pg_yn = ff.pg_yn.value;

	if( ff.claim_reason.selectedIndex == 0  )
	{
		alert('교환/환불 사유를 선택해 주세요.');
		ff.claim_reason.focus();
		return false;
	}
	if(!confirm("교환/환불 접수를 하시겠습니까?")){
		return false;
	}

	// 주문취소 URL 설정
	var exec_url = "/app/mypage/order_claim_cmd";


	$.ajax({
		type: "POST",
		data: "ord_no=" + ord_no + "&claim_reason=" + claim_reason,
		url: exec_url,
		success: function(msg) {
			if(msg == 1) {
				alert("[교환/환불 접수 실패] 관리자에게 문의하시기 바랍니다.");
				opener.location = "/app/mypage/order_view?ord_no=" + ord_no;
				self.close();
			}else {
				alert("교환/환불이 정상적으로 접수 되었습니다.");
				opener.location = "/app/mypage/order_view?ord_no=" + ord_no;
				self.close();
			}
		}
...
function goKwd(kwd){
	...
	callKwdCookie(kwd);
	document.location.href = '/app/product/search?q='+kwd;
}


function com_setCookie(name, value, expires, path, domain, secure){
	var time = new Date();
